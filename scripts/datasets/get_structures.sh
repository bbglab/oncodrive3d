...
if [ -z "$1" ]; then
    echo "[ERROR/FATAL?]: Missing arg1. Specify the path where to save PDB structures."                   #### >> Raise error
    show_help
    exit 1
fi
...
if [ -n "$(ls -A $1/*.tar 2>/dev/null)" ]; then
    ...
    echo "[FATAL]: Could not download .tar file from $AF_URL"                                          #### >> Raise error
fi
...
if [ -n "$(ls -A $1/*.pdb 2>/dev/null)" ]; then
    ...
    if [ "$verbose" = "True" ]; then
        echo "[DEBUG]: Structures downloaded in directory $1"                                  #### >> INFO
    fi
    #touch $1/.checkpoint
else
    echo "[FATAL]: No .pdb files found in directory $1"                                    #### >> Raise error
fi