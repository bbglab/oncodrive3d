...
def summary_plot(gene_result, 
    ...
    # Save
    directory_path = f'{output_dir}/{run_name}'
    filename = f"{run_name}.summary_plot.png"
    plt.savefig(f"{directory_path}/{filename}", dpi=300, bbox_inches='tight')
    print(f"Saved in {directory_path}/{filename}")
    plt.show()
...
def generate_plot(gene_result_path, 
    ...
    seq_df_path = f"{datasets_dir}/seq_for_mut_prob.csv"
